		...
		int was_empty, const char *file, int line, const char *function)
{
	...
	if (ast_taskprocessor_push_origin(pool->control_tps, queued_task_pushed, tpd, file, line, function)) {
		ast_free(tpd);
	}
}
...
#define ast_threadpool_push_internal(pool, task, data) \
	ast_threadpool_push_origin(pool, task, data, __FILE__, __LINE__, __PRETTY_FUNCTION__)
int ast_threadpool_push(struct ast_threadpool *pool, int (*task)(void *data), void *data);

int ast_threadpool_push_origin(struct ast_threadpool *pool, int (*task)(void *data), void *data,
	const char *file, int line, const char *function)
{
	SCOPED_AO2LOCK(lock, pool);
	if (!pool->shutting_down) {
		return ast_taskprocessor_push_origin(pool->tps, task, data, file, line, function);
	}
	return -1;
}
...
int ast_threadpool_push(struct ast_threadpool *pool, int (*task)(void *data), void *data)
{
	return ast_threadpool_push_origin(pool, task, data, NULL, 0, NULL);
}
	...
	const char *file, int line, const char *function)
{
	if (was_empty) {
		...
		if (ast_threadpool_push_origin(ser->pool, execute_tasks, tps, file, line, function)) {
			ast_taskprocessor_unreference(tps);
		}
	}
}
