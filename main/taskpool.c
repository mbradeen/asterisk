...
#define ast_taskpool_push_internal(pool, task, data) \
	ast_taskpool_push_origin(pool, task, data, __FILE__, __LINE__, __PRETTY_FUNCTION__)
int ast_taskpool_push(struct ast_taskpool *pool, int (*task)(void *data), void *data);

int ast_taskpool_push_origin(struct ast_taskpool *pool, int (*task)(void *data), void *data,
	const char *file, int line, const char *function)
{
	...
	if (ast_taskprocessor_push_origin(taskprocessor->taskprocessor, task, data, file, line, function)) {
		return -1;
	}
...
int ast_taskpool_push(struct ast_taskpool *pool, int (*task)(void *data), void *data)
{
	return ast_taskpool_push_origin(pool, task, data, NULL, 0, NULL);
}
...
int ast_taskpool_push_wait_origin(struct ast_taskpool *pool, int (*task)(void *data), void *data,
	const char *file, int line, const char *function)
{
	...
	if (ast_taskpool_push_origin(pool, taskpool_sync_task, &sync_task, file, line, function)) {
...
int ast_taskpool_push_wait(struct ast_taskpool *pool, int (*task)(void *data), void *data)
{
	return ast_taskpool_push_wait_origin(pool, task, data, NULL, 0, NULL);
}
	...
	const char *file, int line, const char *function)
{
	if (was_empty) {
		...
		if (ast_taskpool_push_origin(ser->pool, execute_tasks, tps, file, line, function)) {
			ast_taskprocessor_unreference(tps);
		}
	}
}
